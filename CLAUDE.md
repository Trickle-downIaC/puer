...
- **Memory** (`readMemoryStats`): `sysctl hw.memsize` for total; `host_statistics64(HOST_VM_INFO64)` for the page-count breakdown; `sysctl vm.swapusage` for swap; `sysctl kern.memorystatus_vm_pressure_level` for the kernel pressure verdict. The used/available formula is deliberate and has been the subject of multiple bug fixes (see git log) — `used = appMem + wired + compressed` where `appMem = active - purgeable`, and `available = total - used`. Inactive/speculative/purgeable/free are all treated as reclaimable. Do not "simplify" this without understanding why double-counting inflated the number past physical RAM.
