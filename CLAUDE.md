...
`Puer.xcodeproj` is committed, so opening it needs no tooling; XcodeGen + `project.yml` is only needed to regenerate it. The app target has **no App Sandbox** on purpose — it shells out to `/usr/sbin/ioreg` and `/bin/ps`, which the sandbox would block; do not add a sandbox entitlement.
...
- **Memory** (`readMemoryStats`): `sysctl hw.memsize` for total; `host_statistics64(HOST_VM_INFO64)` for the page-count breakdown; `sysctl vm.swapusage` for swap; `sysctl kern.memorystatus_vm_pressure_level` for the kernel pressure verdict. The used/available formulas are deliberate and have been the subject of multiple bug fixes (see git log) — `Used (Strict) = reserved + wired + app + compressed` where `app = internal - purgeable`, and `Available = purgeable + speculative + file-backed + unallocated` exactly (the full partition model is in the unified-memory section below). Do not "simplify" these without understanding why double-counting inflated the number past physical RAM.
...
- **Per-process memory + recent CPU** (`sampleProcesses` → aggregated in `SystemMonitor.refreshProcesses`): `ps -eo pid,rss,pcpu,comm -r` for the list, then `proc_pid_rusage(RUSAGE_INFO_V4)` per PID for both `ri_phys_footprint` (accurate memory = the "physical footprint" Activity Monitor shows, RSS fallback on failure) and cumulative CPU time. **`ri_user_time`/`ri_system_time` are in mach absolute time units, NOT nanoseconds** — `getProcUsage` converts via `mach_timebase_info` (`machTimebase`); skipping this underreports CPU by ~42× on Apple Silicon (timebase 125/3). Recent CPU% per PID is the CPU-time delta over the wall-clock window (`processWindowSeconds`, 4s), then processes are aggregated by executable name (summing footprint and CPU) with a `(count)` suffix. The "Top CPU" list and the memory list are two views over the same aggregated array.

## Key domain concept

On Apple Silicon there is no separate VRAM: CPU and GPU share one unified memory pool. The allocation bar is an exact partition of physical memory built only from kernel and driver counters: reserved (pages in no named queue; identified live as the firmware carveout the VM system never manages, hw.memsize minus hw.memsize_usable, declared 0.863 GiB against our derived 0.87, the small residue being unqueued kernel pages; the device tree itemizes about two thirds of it as region-size nubs, the ANS storage coprocessor's 600 MiB region at the top of DRAM being the heavyweight, with the rest unadvertised display, SEP, and boot-residue regions per the Asahi Linux boot-protocol map), wired shown as GPU In-Use plus a merged GPU Idle / Non-GPU remainder, app (internal minus purgeable, Activity Monitor's definition), compressed, then the reclaimable tiers of available (purgeable, speculative, file-backed, unallocated; file-backed includes live executable pages, reclaimable only at re-fault cost, so it is not called a cache). A finer GPU-vs-OS split within wired is not publicly measurable and is deliberately not guessed. Used (Strict) is reserved + wired + app + compressed, every tier no reclaim can touch (reserved included: boot-carveout pages are available to nothing, and excluding them silently parks them inside Available); Used (Loose) is exactly Strict + purgeable, the empirical fit to Activity Monitor's Memory Used; Available is exactly the four reclaimable tiers. The driver's "mapped" figure counts address space, can exceed wired or even total, and is shown in the GPU column as a driver claim, not as a bar segment.
