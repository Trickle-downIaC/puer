...
- **"Used (Strict)" headline**, reserved + app + wired + compressed, every tier no reclaim can touch (Available beside it in the status bubble, a Used (Loose) readout beneath, the cache-inclusive convention Activity Monitor and similar monitors report), its five-minute trend, a limits card (GPU wiring against the `iogpu.wired_limit_mb` ceiling, whose headroom is an at-most bound because the limit caps GPU wiring only, beside the live kernel pressure verdict and last-event clock), and an allocation card where the five primary readouts (Reserved, App per Activity Monitor's internal-minus-purgeable definition, Wired, Compressed, Available) caption an exact-partition chart with its cache tiers in the legend
